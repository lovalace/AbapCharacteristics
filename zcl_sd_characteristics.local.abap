...
PRIVATE SECTION.
CLASS-METHODS GetList IMPORTING it_data TYPE type=>tt_data RETURNING VALUE(r_result) TYPE type=>tt_ausp.
CLASS-METHODS: GetMatnrAndCharg IMPORTING table TYPE REF TO data
                                map_matnr TYPE fieldname OPTIONAL
                                map_charg TYPE Fieldname OPTIONAL
                                RETURNING VALUE(Result) TYPE type=>tt_data.
CLASS-DATA: _programName TYPE zbct002-programname. "zbct002 --> custom configuration table
ENDCLASS.
...
CLASS Core IMPLEMENTATION.
  ...
  METHOD GetList.
 "zmm_cds_char --> ausp table cds view.  "zbct002 -->  list of characteristics table
   SELECT  DISTINCT _ausp~matnr, _ausp~charg, _ausp~atnam, _ausp~atflv,_ausp~atwrt,_ausp~atfor,_ausp~anzst,_ausp~anzdz
           FROM zmm_cds_char AS _ausp FOR ALL ENTRIES IN @it_data
           WHERE _ausp~matnr = @it_data-matnr AND
                 _ausp~charg = @it_data-charg AND
                 _ausp~atnam in ( SELECT atnam FROM zbct002 INNER JOIN cabn ON zbct002~atinn = cabn~atinn  WHERE zbct002~programname = @_programname )
...
CLASS-METHODS: TransposeData IMPORTING map_matnr TYPE fieldname OPTIONAL
               ...
               setadditionfield CHANGING  fieldcatalog TYPE lvc_t_fcat.
PRIVATE SECTION.
 TYPES: tt_Fields TYPE TABLE OF zbct007 WITH EMPTY KEY.
  CLASS-METHODS CreateFieldCatalog IMPORTING i_table TYPE REF TO data RETURNING VALUE(Result) TYPE lvc_t_fcat.
  CLASS-METHODS addcharacteristicsfcat CHANGING fieldcatalog TYPE lvc_t_fcat
  ...
  CLASS-METHODS createitab CHANGING fieldcatalog TYPE lvc_t_fcat
                                    table        TYPE data.
  CLASS-METHODS SetTextFieldFieldcatalog IMPORTING i_salv TYPE REF TO cl_salv_table
                                         CHANGING  c_result TYPE lvc_t_fcat.
  CLASS-METHODS getfields RETURNING value(result) TYPE tt_Fields.
  CLASS-METHODS SetFields IMPORTING  fieldnames TYPE business=>tt_fields
...
*    DD04T
 ...
 """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
   ...
   ENDLOOP.
  "Clear Referance fields for check value etc.
